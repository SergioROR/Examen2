...
router.post("/api/login", async (req, res) => {
  ...
  try {
    const result = await db.query(
      "SELECT id_usuario, correo, password, nombre, rol, id_plantel, estado, imagen FROM usuarios WHERE correo = $1",
      [correo]
    );
    ...
    // ✅ Verificar si está inhabilitado ANTES de comparar contraseña
...
router.post("/api/usuario", async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO usuarios (nombre, apellidos, correo, password, rol, id_plantel, imagen) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
